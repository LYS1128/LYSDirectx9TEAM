#include "stdafx.h"
#include "..\Public\CokeGolem.h"

CCokeGolem::CCokeGolem(LPDIRECT3DDEVICE9 pGraphic_Device)
	:CMonster(pGraphic_Device)
{
}

CCokeGolem::CCokeGolem(CCokeGolem & rhs)
	: CMonster(rhs)
{
}

HRESULT CCokeGolem::NativeConstruct_Prototype()
{
	if (FAILED(__super::NativeConstruct_Prototype())) {
		MSG_BOX(L"Failed To CCokeGolem : NativeConstruct_Prototype");
		return E_FAIL;
	}
...
HRESULT CCokeGolem::NativeConstruct(void * pArg)
{
	if (FAILED(__super::NativeConstruct(pArg))) {
		MSG_BOX(L"Failed To CCokeGolem : NativeConstruct");
		return E_FAIL;
	}
	...
	m_pTransform->Scaled(_float3(3.5f, 3.5f, 3.5f));
...
void CCokeGolem::Tick(_float fTimeDelta)
{
...
void CCokeGolem::LateTick(_float fTimeDelta)
{
...
HRESULT CCokeGolem::Render()
{
	if (FAILED(__super::Render())) {
		MSG_BOX(L"Failed To CCokeGolem : Render");
		return E_FAIL;
	}
	return S_OK;
}

HRESULT CCokeGolem::SetUp_Components()
{
	...
	if (FAILED(__super::SetUp_Components(TEXT("Com_Transform"), LEVEL_STATIC, TEXT("Prototype_Component_Transform"), (CComponent**)&m_pTransform, &TransformDesc))) {
		MSG_BOX(L"Failed To CCokeGolem : SetUp_Components");
		return E_FAIL;
	}
	if (FAILED(__super::SetUp_Components(TEXT("Com_Texture_GhostStump"), LEVEL_STAGESIX, TEXT("Prototype_Component_Texture_CokeGolem"), (CComponent**)&m_pTexture))) {
		MSG_BOX(L"Failed To CCokeGolem : SetUp_Components");
		return E_FAIL;
	}
...
CCokeGolem * CCokeGolem::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CCokeGolem*	pInstance = new CCokeGolem(pGraphic_Device);
...
CGameObject * CCokeGolem::Clone(void * pArg)
{
	CCokeGolem*	pInstance = new CCokeGolem(*this);
