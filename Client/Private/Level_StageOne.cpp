...
HRESULT CLevel_StageOne::NativeConstruct() {
	if (FAILED(__super::NativeConstruct())) {
		MSG_BOX(L"Failed To CLevel_StageOne : NativeConstruct");
		return E_FAIL;
	}
	...
	g_iLevel = LEVEL_STAGEONE;

	return S_OK;
}

void CLevel_StageOne::Tick(_float fTimeDelta) {

	CGameInstance* pGameInstance = CGameInstance::GetInstance();
	Safe_AddRef(pGameInstance);

	CSound_Manager::GetInstance()->PlaySound(L"Stage_1.wav", CSound_Manager::BGM, VOLUME);

	if (pGameInstance->Key_Down('0')) {
		if (FAILED(pGameInstance->OpenLevel(LEVEL_LOADING, CLevel_Loading::Create(m_pGraphic_Device, LEVEL_STAGETWO)))) {
			return;
		}
	}
...
HRESULT CLevel_StageOne::Render() {
	if (FAILED(__super::Render())) {
		MSG_BOX(L"Failed To CLevel_StageOne : Render");
		return E_FAIL;
	}
	//SetWindowText(g_hWnd, TEXT("1스테이지입니다."));
	return S_OK;
}
...
HRESULT CLevel_StageOne::Ready_Layer_Portal(const _tchar* pLayerTag) {
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
	Safe_AddRef(pGameInstance);

	dynamic_cast<CTransform*>(pGameInstance->Find_Layer(LEVEL_STATIC, L"Layer_Portal")->Get_Component(L"Com_Transform"))->Set_State(CTransform::STATE_POSITION, _float3(18.5f, 2.5f, 8.5f));
...
HRESULT CLevel_StageOne::Ready_Layer_BackGround(const _tchar* pLayerTag) {
	if (FAILED(Load_Map(L"../../Data/StageOneMap.dat"))) {
		MSG_BOX(L"Failed To CLevel_StageOne : Ready_Layer_BackGround");
		return E_FAIL;
	}
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
	Safe_AddRef(pGameInstance);
	...
	for (auto iter : m_MapCube) {
		if (FAILED(pGameInstance->Add_GameObjectToLayer(LEVEL_STAGEONE, pLayerTag, L"Prototype_GameObject_Ground",&iter))) {
			MSG_BOX(L"Failed To CLevel_StageOne : Ready_Layer_BackGround");
			return E_FAIL;
		}
	}
	if (FAILED(pGameInstance->Add_GameObjectToLayer(LEVEL_STAGEONE, pLayerTag, TEXT("Prototype_GameObject_Sky")))) {
		MSG_BOX(L"Failed To CLevel_StageOne : Ready_Layer_BackGround");
		return E_FAIL;
	}
	Safe_Release(pGameInstance);
...
HRESULT CLevel_StageOne::Ready_Layer_Monster(const _tchar* pLayerTag) {
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
	Safe_AddRef(pGameInstance);
	...
	tMonster.vPosition = _float3(4.f,10.f,13.f);

	if (FAILED(pGameInstance->Add_GameObjectToLayer(LEVEL_STAGEONE, pLayerTag, TEXT("Prototype_GameObject_Gourd"), &tMonster))) {
...
HRESULT CLevel_StageOne::Ready_Layer_NPC(const _tchar* pLayerTag) {
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
	Safe_AddRef(pGameInstance);

	CNPC::NPC tNPC;
	tNPC.pTargetTransform = (CTransform*)pGameInstance->Get_Component(LEVEL_STATIC, L"Layer_Player", L"Com_Transform");
	tNPC.vPosition = _float3(18.5f, 2.5f, 4.5f);

	if (FAILED(pGameInstance->Add_GameObjectToLayer(LEVEL_STAGEONE, pLayerTag, TEXT("Prototype_GameObject_Magician"), &tNPC))) {
...
CLevel_StageOne* CLevel_StageOne::Create(LPDIRECT3DDEVICE9 pGraphic_Device) {
	...
	if (FAILED(pInstance->NativeConstruct())) {
		MSG_BOX(L"Failed To CLevel_StageOne : Create");
...
HRESULT CLevel_StageOne::Load_Map(const TCHAR* pPath) {
	HANDLE hFile = CreateFile(pPath, GENERIC_READ, 0, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);

	if (INVALID_HANDLE_VALUE == hFile) {
		MSG_BOX(L"Failed To CLevel_StageOne : Load_Map");
		return E_FAIL;
	}
	DWORD dwByte = 0;
